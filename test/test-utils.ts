...
export function createMockStreamEvent(
        records: Array<{
                ...
                dynamodb: {
                        ...
                        NewImage?: Record<string, { S?: string; N?: string; B?: string; L?: unknown[]; M?: Record<string, unknown> }>;
                        OldImage?: Record<string, { S?: string; N?: string; B?: string; L?: unknown[]; M?: Record<string, unknown> }>;
                };
        }>,
): DynamoDBStreamEvent {
        const dynamoDBRecords: DynamoDBRecord[] = records.map((record) => ({
                eventID: `event_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`,
