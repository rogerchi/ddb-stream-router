...
> {
        ...
        onRemove<T>(
                ...
                options: BatchHandlerOptions & RemoveHandlerOptions,
        ): HandlerRegistration<V>;
        onRemove<T>(
                matcher: Matcher<T>,
        ...
        onRemove<T>(
                ...
                options?: RemoveHandlerOptions | (BatchHandlerOptions & RemoveHandlerOptions),
        ): HandlerRegistration<V> {
                return this.router.onRemove(
                        matcher,
                        handler as RemoveHandler<T, V>,
                        options as RemoveHandlerOptions,
                );
        }

        onTTLRemove<T>(
                matcher: Matcher<T>,
                handler: BatchTTLRemoveHandler<T, V>,
                options: BatchHandlerOptions,
        ): HandlerRegistration<V>;
        onTTLRemove<T>(
                matcher: Matcher<T>,
                handler: TTLRemoveHandler<T, V>,
                options?: HandlerOptions,
        ): HandlerRegistration<V>;
        onTTLRemove<T>(
                matcher: Matcher<T>,
                handler: TTLRemoveHandler<T, V> | BatchTTLRemoveHandler<T, V>,
                options?: HandlerOptions | BatchHandlerOptions,
        ): HandlerRegistration<V> {
                return this.router.onTTLRemove(
                        matcher,
                        handler as TTLRemoveHandler<T, V>,
                        options as HandlerOptions,
                );
        }
...
export class StreamRouter<V extends StreamViewType = "NEW_AND_OLD_IMAGES"> {
        ...
        /**
         * Checks if a REMOVE event was triggered by TTL expiration.
         * TTL removals are identified by userIdentity.type === "Service" and
         * userIdentity.principalId === "dynamodb.amazonaws.com"
         */
        isTTLRemoval(record: DynamoDBRecord): boolean {
                const userIdentity = record.userIdentity as
                        | {
                                        type?: string;
                                        principalId?: string;
                          }
         ...
         * - Event type (INSERT, MODIFY, REMOVE, TTL_REMOVE)
        ...
        /**
         * Register a handler for TTL-triggered REMOVE events.
         * Only handles removals where userIdentity.type === "Service" and
         * userIdentity.principalId === "dynamodb.amazonaws.com"
         * @overload Batch mode - handler receives array of records
         */
        onTTLRemove<T>(
        ...
        async process(
                ...
                for (const record of event.Records) {
                        ...
                        try {
                                ...
                                // For REMOVE events, we need to consider both REMOVE and TTL_REMOVE handlers
